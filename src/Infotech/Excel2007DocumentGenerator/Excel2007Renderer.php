<?php
/*
 * This file is part of the infotech/excel2007-document-generator package.
 *
 * (c) Infotech, Ltd
 *
...
use Infotech\DocumentGenerator\Renderer\RenderException;
use ZipArchive;
...
class Excel2007Renderer implements RendererInterface
{
     ...
     * @param array $data
     * @throws \RenderException While template read error or temporary document write error
    ...
    public function render($templatePath, array $data)
    {
        ...
        if (!@copy($templatePath, $tmpPath)) {
            throw new RenderException('Error while reading a template file');
        }

        $zip = new ZipArchive();
        if (!$zip->open($tmpPath) || !$sharedStrings = $zip->getFromName('xl/sharedStrings.xml')) {
            throw new RenderException('Unsupported template file format. Required xlsx (Excel2007) file');
        }
        ...
        if (!$zip->close() ) {
            throw new RenderException('Error while writing temporary rendered file');
        }
